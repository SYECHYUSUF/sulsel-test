<section 
    ...
    class="relative py-16 md:py-24 bg-white overflow-hidden font-['Plus_Jakarta_Sans']">
    
    {{-- Parallax Background Elements --}}
    <div class="absolute top-0 right-0 w-72 h-72 bg-violet-100 rounded-full blur-[100px] opacity-40 -translate-y-1/2 translate-x-1/3 transition-transform duration-75 ease-out"
         :style="`transform: translate(${30 + (scroll * 0.03)}%, ${-30 + (scroll * 0.03)}%)`"></div>
    <div class="absolute bottom-0 left-0 w-72 h-72 bg-purple-100 rounded-full blur-[100px] opacity-40 translate-y-1/2 -translate-x-1/3 transition-transform duration-75 ease-out"
         :style="`transform: translate(${-30 - (scroll * 0.03)}%, ${30 - (scroll * 0.03)}%)`"></div>

    <div class="container mx-auto px-6 relative z-10">
        {{-- Section Header --}}
        <div class="text-center mb-16 md:mb-20" data-aos="fade-down">
            <span class="text-violet-600 font-bold tracking-[0.2em] text-[10px] uppercase mb-3 block">Pimpinan Daerah</span>
            <h2 class="text-3xl md:text-4xl font-extrabold text-gray-900 mb-4 tracking-tight">Gubernur & Wakil Gubernur</h2>
            <div class="w-12 h-1 bg-violet-600 mx-auto rounded-full"></div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-12 max-w-4xl mx-auto">
            ...
            <div data-aos="fade-up" data-aos-delay="{{ $index * 200 }}" class="flex flex-col group">
                ...
                {{-- 1. Image Card --}}
                <div class="relative aspect-[4/5] w-full rounded-[2.5rem] md:rounded-[3rem] overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.04)] border border-gray-50 transition-all duration-700 group-hover:shadow-xl group-hover:shadow-violet-100/50 group-hover:-translate-y-1">
                    
                    {{-- === BARU: List Ungu di Atas Card dengan Corner Radius === --}}
                    <div class="absolute top-0 inset-x-0 h-2 bg-violet-600 z-40 rounded-t-lg"></div>
                    {{-- ======================================================== --}}
                    ...
                    <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-20"></div>
                    ...
                    <div class="absolute bottom-6 left-6 z-30 flex gap-2 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 ease-out">
                        <a href="#" class="w-9 h-9 bg-white/20 backdrop-blur-md border border-white/30 text-white rounded-xl flex items-center justify-center hover:bg-violet-600 hover:border-violet-600 transition-all transform hover:scale-110">
                ...
                <div class="mt-5 text-center md:text-left md:pl-2">
                    <h3 class="text-lg md:text-xl font-bold text-gray-900 mb-0.5 tracking-tight group-hover:text-violet-600 transition-colors">
                        {{ $l['name'] }}
                    </h3>
                    <p class="text-[10px] md:text-xs text-violet-600 font-bold uppercase tracking-[0.15em]">
                        {{ $l['pos'] }}
                    </p>
                </div>
