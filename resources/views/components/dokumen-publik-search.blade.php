         ...
         <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl ring-1 ring-black/5 overflow-hidden">
            ...
            <div x-show="results.length > 0 || isLoading" class="max-h-[60vh] overflow-y-auto custom-scrollbar">
                ...
                <div x-show="isLoading" class="p-8 text-center text-gray-500 dark:text-gray-400">
                    <div class="flex flex-col items-center justify-center">
                        <svg class="animate-spin h-8 w-8 text-ppid-accent mb-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        ...
                        <a :href="'/informasi-publik/detail/' + item.id_informasi" class="block px-4 py-3 mx-2 rounded-xl hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors group">
                            <div class="flex items-start gap-3">
                                <div class="mt-1 shrink-0">
                                    <div class="w-8 h-8 rounded-lg bg-ppid-primary/10 dark:bg-white/5 flex items-center justify-center text-ppid-primary dark:text-gray-300 group-hover:bg-ppid-primary group-hover:text-white transition-colors">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2 mb-0.5">
                                        <h4 class="text-sm font-semibold text-gray-900 dark:text-white truncate group-hover:text-ppid-primary dark:group-hover:text-ppid-accent" x-text="item.judul"></h4>
            ...
            <div x-show="!searchQuery && results.length === 0" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                ...
                <div class="mt-4 flex flex-wrap justify-center gap-2">
                    <button @click="searchQuery = 'RKA'; fetchSuggestions(); $refs.searchInput.focus()" class="px-3 py-1 text-xs rounded-full border border-gray-200 dark:border-slate-700 hover:border-ppid-accent hover:text-ppid-accent transition-colors">#RKA</button>
                    <button @click="searchQuery = 'Anggaran'; fetchSuggestions(); $refs.searchInput.focus()" class="px-3 py-1 text-xs rounded-full border border-gray-200 dark:border-slate-700 hover:border-ppid-accent hover:text-ppid-accent transition-colors">#Anggaran</button>
                    <button @click="searchQuery = 'Lapor'; fetchSuggestions(); $refs.searchInput.focus()" class="px-3 py-1 text-xs rounded-full border border-gray-200 dark:border-slate-700 hover:border-ppid-accent hover:text-ppid-accent transition-colors">#Lapor</button>
                </div>
            </div>
