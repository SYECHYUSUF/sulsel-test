<x-layout>
        ...
        get filteredPengajuan() {
            ...
            return this.pengajuan.filter(item => {
                if (this.statusFilter === 'belum_direspon') {
                    return item.display_status === 'belum_direspon';
                }
            ...
            <div class="max-w-4xl mx-auto">
                <div class="bg-gradient-to-br from-white via-slate-50 to-blue-50/30 dark:from-slate-800 dark:via-slate-800 dark:to-slate-900 rounded-[2.5rem] shadow-2xl p-8 md:p-14 border-2 border-[#1A305E]/10 dark:border-blue-500/30">
                    ...
                    <div class="text-center mb-10">
                        ...
                        <p class="text-slate-600 dark:text-slate-400">Gunakan email yang sama saat mengajukan pengajuan keberatan</p>
                    </div>
                    ...
                    <form @submit.prevent="searchPengajuan" class="max-w-2xl mx-auto">
                        ...
                        <div class="mb-8">
                            ...
                            <div class="relative">
                                <select x-model="statusFilter" @change="if(email && pengajuan.length > 0) searchPengajuan()"
                                ...
                                <div class="absolute right-6 top-1/2 -translate-y-1/2 pointer-events-none text-[#1A305E] dark:text-white">
    ...
    {{-- Results Section --}}
    <section id="hasil-pencarian" x-show="pengajuan.length > 0" x-cloak class="py-12 md:py-20 bg-slate-50 dark:bg-slate-900">
        <div class="container mx-auto px-4 sm:px-6">
            <div class="text-center mb-12">
                <div class="inline-flex flex-col sm:flex-row items-center gap-4 bg-white dark:bg-slate-800 px-8 py-5 rounded-2xl shadow-lg border-2 border-[#1A305E]/10">
                    ...
                    <span class="bg-[#D4AF37] text-white px-4 py-1 rounded-full text-sm font-bold" x-text="filteredPengajuan.length + ' Data'"></span>
                </div>
            </div>
                    ...
                    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border-2 border-slate-100 dark:border-slate-700 overflow-hidden">
                        ...
                        <div class="p-6 bg-slate-50 dark:bg-slate-700/30 border-b-2 border-slate-100 dark:border-slate-700 flex flex-wrap justify-between items-center gap-4">
                            <div class="flex items-center gap-4">
                                <div class="p-3 bg-[#1A305E] rounded-xl shadow-md">
                                    <svg class="w-7 h-7 text-[#D4AF37]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                                </div>
                                <div>
                            ...
                            <span class="px-5 py-2 rounded-xl text-sm font-bold text-white shadow-md" 
                                :class="{
                                    'bg-red-500': item.display_status == 'belum_direspon',
                        ...
                        <div class="p-8 grid lg:grid-cols-2 gap-8">
                            ...
                            <div>
                                ...
                                <p class="text-lg font-bold text-[#1A305E] dark:text-white leading-relaxed mb-6" x-text="item.nama_pemohon"></p>
                                
                                <label class="flex items-center gap-2 text-xs font-bold text-[#D4AF37] uppercase tracking-wider mb-3">
                                    <span>Kasus Posisi</span>
                                </label>
                                <p class="text-base text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-line" x-text="item.kasus"></p>
                            </div>

                            {{-- Right: Response --}}
                            <div class="space-y-6">
                                <div>
                                    <label class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 block">Tanggapan Admin</label>
                                    <div class="bg-slate-50 dark:bg-slate-900 rounded-2xl p-6 border-2 border-slate-100 dark:border-slate-700 min-h-[100px]">
                                        ...
                                        <template x-if="item.status == 't'">
                                            <div class="text-red-600 font-medium">
                                                ...
                                                <p class="text-sm" x-text="item.feedback || 'Silakan hubungi admin untuk informasi lebih lanjut.'"></p>
                                            </div>
                                        </template>
                                        ...
                                        <template x-if="item.tgl_feedback">
                                            <div class="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                                                <p class="text-xs text-slate-500">
                                                    Dijawab pada: <span x-text="new Date(item.tgl_feedback).toLocaleDateString('id-ID', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })"></span>
                                                    <template x-if="item.feedback_by">
                                                        <span> oleh <span class="font-semibold" x-text="item.feedback_by.name"></span></span>
                                                    </template>
                                                </p>
                                            </div>
                                        </template>
