<x-admin-layout>
    <x-slot:title>Edit Media Sosial</x-slot:title>

    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">Edit Media Sosial</h1>
        ...
        <div
            ...
            <form action="{{ route('admin.social-links.update', $sosmed->id_sosmed) }}" method="POST"
                class="p-6 space-y-4">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="sosmed" id="sosmed" value="{{ old('sosmed', $sosmed->sosmed) }}"
                            required
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="judul" id="judul" value="{{ old('judul', $sosmed->judul) }}"
                    ...
                    <div class="space-y-2 md:col-span-2">
                        <label for="link_sosmed"
                            class="block text-sm font-medium text-slate-700 dark:text-slate-300">Tautan (URL) <span
                                class="text-red-500">*</span></label>
                        <input type="url" name="link_sosmed" id="link_sosmed"
                            value="{{ old('link_sosmed', $sosmed->link_sosmed) }}" required
                    ...
                    <div class="space-y-2 md:col-span-2"
                        x-data="{ selectedIcon: '{{ old('icon_sosmed', $sosmed->icon_sosmed) }}' }">
                            ...
                            <div class="flex-1">
                                <select name="icon_sosmed" id="icon_sosmed" required x-model="selectedIcon"
                                    ...
                                    @foreach($predefinedIcons as $name => $path)
                                        <option value="{{ $path }}" {{ $sosmed->icon_sosmed == $path ? 'selected' : '' }}>
                                            {{ $name }}</option>
                                    @endforeach
                        ...
                        <div class="mt-4" x-show="selectedIcon === 'custom'">
                            ...
                            <textarea name="custom_icon" id="custom_icon" rows="2"
                                ...
                                placeholder='Contoh: <path d="..." />'>{{ !in_array($sosmed->icon_sosmed, $predefinedIcons) ? $sosmed->icon_sosmed : '' }}</textarea>
                        </div>
                    ...
                    <div class="space-y-2">
                        <label for="urutan" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Urutan
                            Tampil</label>
                        <input type="number" name="urutan" id="urutan" value="{{ old('urutan', $sosmed->urutan) }}"
                            class="w-full px-4 py-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                            placeholder="1, 2, 3...">
                    </div>
                </div>
