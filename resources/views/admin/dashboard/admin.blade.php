<x-admin-layout title="Dashboard Admin - PPID Sulsel">
    ...
    : number_format($stats['total_visitor']) 
        ...
        <div class="grid grid-cols-12 gap-6">
            ...
            <!-- Menunggu Verifikasi Berita -->
            <div
                class="col-span-12 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    <div>
                        <h3 class="text-slate-800 dark:text-slate-100 font-bold text-lg">Menunggu Verifikasi Berita</h3>
                        <p class="text-xs text-slate-500 dark:text-slate-400">Berita terbaru yang memerlukan tinjauan admin</p>
                    </div>
                    <a href="{{ route('admin.berita.index', ['verify' => 'n']) }}"
                        class="text-xs font-semibold text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 px-3 py-1.5 bg-blue-50 dark:bg-blue-900/20 rounded-lg transition-colors">
                        Tampilkan Semua
                    </a>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        <thead>
                            <tr class="border-b border-slate-100 dark:border-slate-700">
                                <th class="pb-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Judul Berita</th>
                                <th class="pb-4 text-xs font-bold text-slate-400 uppercase tracking-wider hidden lg:table-cell">SKPD Penginput</th>
                                <th class="pb-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                            @forelse($pendingBerita as $berita)
                                <tr class="group hover:bg-slate-50 dark:hover:bg-slate-900/40 transition-colors">
                                    <td class="py-4 pr-4">
                                        <div class="flex items-start gap-3">
                                            <div class="mt-1 p-2 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg group-hover:bg-blue-100 transition-colors">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z"></path>
                                                </svg>
                                            </div>
                                            <div class="min-w-0">
                                                <p class="text-sm font-bold text-slate-800 dark:text-slate-200 line-clamp-1">{{ $berita->judul }}</p>
                                                <p class="text-[10px] text-slate-500 flex items-center gap-1 mt-0.5">
                                    ...
                                    <td class="py-4 hidden lg:table-cell">
                                        <span class="text-xs text-slate-600 dark:text-slate-400 font-medium truncate max-w-[150px]">
                                            {{ $berita->skpd->nm_skpd ?? '-' }}
                                        </span>
                                    </td>
                                    <td class="py-4">
            ...
            <!-- Menunggu Verifikasi IKPHN -->
            <div
                class="col-span-12 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    <div>
                        <h3 class="text-slate-800 dark:text-slate-100 font-bold text-lg">Menunggu Verifikasi IKPHN</h3>
                        <p class="text-xs text-slate-500 dark:text-slate-400">Data IKPHN terbaru yang memerlukan tinjauan admin</p>
                    </div>
                    <a href="{{ route('admin.ikphns.index', ['verify' => 'n']) }}"
                        class="text-xs font-semibold text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 px-3 py-1.5 bg-blue-50 dark:bg-blue-900/20 rounded-lg transition-colors">
                        Tampilkan Semua
                    </a>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        <thead>
                            <tr class="border-b border-slate-100 dark:border-slate-700">
                                <th class="pb-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Nama Jabatan</th>
                                <th class="pb-4 text-xs font-bold text-slate-400 uppercase tracking-wider hidden lg:table-cell">SKPD Penginput</th>
                                <th class="pb-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                            @forelse($pendingIkphn as $ikphn)
                                <tr class="group hover:bg-slate-50 dark:hover:bg-slate-900/40 transition-colors">
                                    <td class="py-4 pr-4">
                                        <div class="flex items-start gap-3">
                                            <div class="mt-1 p-2 bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 rounded-lg group-hover:bg-purple-100 transition-colors">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                                                </svg>
                                            </div>
                                            <div class="min-w-0">
                                                <p class="text-sm font-bold text-slate-800 dark:text-slate-200 line-clamp-1">{{ $ikphn->nama_jabatan }}</p>
                                                <p class="text-[10px] text-slate-500 flex items-center gap-1 mt-0.5">
                                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                                    </svg>
                                                    {{ $ikphn->created_at ? $ikphn->created_at->format('d M Y') : '-' }}
                                                </p>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-4">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('admin.ikphns.edit', $ikphn->id) }}"
                                                class="text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
                                                title="Verifikasi">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="py-12 text-center">
                                        <div class="flex flex-col items-center gap-2 text-slate-400">
                                            <p class="text-sm italic">Tidak ada data IKPHN menunggu verifikasi.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
