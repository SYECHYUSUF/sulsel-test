    ...
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center gap-4 mb-6">
            <a href="{{ route('admin.data-sop.index') }}"
                class="p-2 bg-white border border-slate-200 rounded-xl text-slate-400 hover:text-slate-600 transition-all">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Edit SOP</h1>
                <p class="text-slate-500 text-sm">Perbarui informasi atau dokumen SOP.</p>
            </div>
        </div>
        ...
        <form action="{{ route('admin.data-sop.update', $sop->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
                <div>
                    <label for="judul" class="block text-sm font-semibold text-slate-700 mb-2">Judul Dokumen SOP <span
                            class="text-rose-500">*</span></label>
                    <input type="text" name="judul" id="judul" value="{{ old('judul', $sop->judul) }}"
                        class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all"
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">File Dokumen</label>
                    <div class="mb-4 p-4 bg-slate-50 border border-slate-100 rounded-xl flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <a href="{{ asset('storage/sop/' . $sop->file) }}" target="_blank"
                                class="text-xs text-indigo-600 hover:underline">{{ $sop->file }}</a>
                        </div>
                    </div>

                    <input type="file" name="file" id="file" class="filepond">
                    <p class="mt-2 text-xs text-slate-500">Biarkan kosong jika tidak ingin mengubah file. Format: PDF,
            ...
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('admin.data-sop.index') }}"
                    class="px-6 py-2.5 border border-slate-200 rounded-xl text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-all">Batal</a>
                <button type="submit"
