<x-admin-layout>
    ...
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center gap-4 mb-6">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Input SOP Baru</h1>
                <p class="text-slate-500 text-sm">Silakan lengkapi formulir di bawah ini untuk menambahkan SOP baru.</p>
            </div>
        </div>
        ...
        <form action="{{ route('admin.data-sop.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">File Dokumen <span
                            class="text-rose-500">*</span></label>
                    <input type="file" name="file" id="file" class="filepond" required>
                    <p class="mt-2 text-xs text-slate-500">Format yang didukung: PDF, DOC, DOCX, XLS, XLSX, JPG, PNG.
                        Maksimal 5MB.</p>
                    @error('file')
                        <p class="mt-1 text-sm text-rose-500">{{ $message }}</p>
                    @enderror
