    ...
    <div x-data="newsDataTable()" x-init="fetchData()" class="space-y-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-slate-500 dark:text-slate-400 text-sm">Kelola informasi dan berita sektoral SKPD.</p>
            </div>
            <div class="flex items-center gap-3">
                <div class="relative">
                    <input 
                        ...
                        class="pl-10 pr-4 py-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 w-full md:w-64 text-sm transition-all"
                    >
                        ...
                        <template x-if="loading">
                            <template x-for="i in 5" :key="i">
                                <tr class="animate-pulse">
                                    ...
                                    <td class="px-6 py-4"><div class="h-4 bg-slate-100 dark:bg-slate-700 rounded w-24"></div></td>
                                    <td class="px-6 py-4 text-right"><div class="h-8 bg-slate-100 dark:bg-slate-700 rounded-lg w-12 ml-auto"></div></td>
                                </tr>
                            </template>
                        </template>
                        ...
                        <template x-if="!loading">
                            <template x-for="item in news" :key="item.id_berita">
                                <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors">
                                    <td class="px-6 py-4">
                                        <div class="font-medium text-slate-900 dark:text-slate-100 line-clamp-1" x-text="item.judul"></div>
                                    ...
                                    <td class="px-6 py-4">
                                        <span :class="{
                                            'bg-emerald-50 text-emerald-700 border-emerald-100 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800': item.verify === 'y',
                                            'bg-rose-50 text-rose-700 border-rose-100 dark:bg-rose-900/30 dark:text-rose-400 dark:border-rose-800': item.verify === 'n',
                                            'bg-amber-50 text-amber-700 border-amber-100 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800': item.verify === 't'
                                        }" class="px-2.5 py-1 rounded-full text-xs font-medium border" x-text="getStatusLabel(item.verify)"></span>
                                    </td>
                                    <td class="px-6 py-4 text-slate-500 dark:text-slate-400" x-text="formatDate(item.tgl_upload)"></td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex justify-end gap-2">
                                            <a :href="'/admin/berita/' + item.id_berita + '/edit'" class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 dark:hover:text-indigo-400 rounded-lg transition-all">
        ...
        <div class="flex flex-col md:flex-row items-center justify-between gap-4 py-2" x-show="!loading && news.length > 0">
            <div class="text-sm text-slate-500 dark:text-slate-400">
                Menampilkan <span class="font-medium text-slate-900 dark:text-slate-100" x-text="news.length"></span> data dari <span class="font-medium text-slate-900 dark:text-slate-100" x-text="pagination.total"></span> total berita
            </div>
            <div class="flex items-center gap-2">
                <button 
                    @click="changePage(pagination.prev_page_url)" 
                    :disabled="!pagination.prev_page_url"
                    class="px-4 py-2 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                    Sebelumnya
                </button>
                <button 
