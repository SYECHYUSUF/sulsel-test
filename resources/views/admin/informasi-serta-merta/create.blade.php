<x-admin-layout title="Tambah Informasi Serta Merta - Admin PPID">
    <x-slot name="header">
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.informasi-serta-merta.index') }}" class="text-slate-500 hover:text-slate-700">
            ...
            <span>Tambah Informasi Serta Merta</span>
        </div>
    </x-slot>

    <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden max-w-4xl mx-auto">
        <div class="p-6 border-b border-slate-100">
            <h3 class="text-lg font-bold text-ppid-primary">Form Tambah Data</h3>
        </div>

        <form action="{{ route('admin.informasi-serta-merta.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div class="pt-6 border-t border-slate-100 flex justify-end gap-3">
                <a href="{{ route('admin.informasi-serta-merta.index') }}"
