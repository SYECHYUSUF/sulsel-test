<x-admin-layout>
    <x-slot:title>Master Data Pekerjaan</x-slot:title>
    ...
    <div x-data="{
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">Master Data Pekerjaan</h1>
                <p class="text-slate-500 dark:text-slate-400 text-sm">Kelola data pekerjaan untuk formulir permohonan informasi</p>
            </div>
            <a href="{{ route('admin.master-pekerjaan.create') }}" 
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        @forelse($data as $index => $item)
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div class="font-medium text-slate-900 dark:text-slate-100">{{ $item->nama_pekerjaan }}</div>
                                </td>
                                <td class="px-6 py-4">
                                    @if($item->is_active)
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        <a href="{{ route('admin.master-pekerjaan.edit', $item->id) }}" 
                                        ...
                                        <form action="{{ route('admin.master-pekerjaan.destroy', $item->id) }}" method="POST" class="inline" 
                                              @submit.prevent="openDeleteModal($el, '{{ $item->nama_pekerjaan }}')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" 
        ...
        @if($data->count() > 0)
            <div class="text-sm text-slate-500 dark:text-slate-400">
                Total: <span class="font-medium text-slate-900 dark:text-slate-100">{{ $data->count() }}</span> pekerjaan
            </div>
        @endif

        {{-- Delete Confirmation Modal --}}
        <x-delete-confirmation>
            Apakah Anda yakin ingin menghapus pekerjaan <span class="font-bold text-slate-800 dark:text-white" x-text="deleteItemName"></span>?
        </x-delete-confirmation>
    </div>
</x-admin-layout>
