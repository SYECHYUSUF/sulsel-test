<x-admin-layout title="Disposisi Pengajuan Keberatan - Admin PPID">
    <x-slot name="header">
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.pengajuan-keberatan.show', $pengajuan->id_pengajuan) }}" class="text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-300">
            ...
            <span>Disposisi Pengajuan Keberatan</span>
        </div>
    </x-slot>
    ...
    <div class="max-w-5xl mx-auto" 
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden mb-6">
            <div class="bg-gradient-to-r from-[#1A305E] to-blue-700 p-6 text-white">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1">
                        <h1 class="text-2xl font-bold mb-1">Disposisi Pengajuan Keberatan</h1>
                        <p class="text-blue-100 text-sm">Pilih OPD yang akan menangani keberatan ini</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden mb-6">
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    <label class="text-xs text-slate-500 dark:text-slate-400 uppercase font-semibold">Kasus/Alasan Keberatan</label>
                    <p class="text-slate-900 dark:text-slate-100 font-medium">{{ $pengajuan->kasus ?? '-' }}</p>
                </div>
            </div>
        </div>

        {{-- SKPD Selection Form --}}
        <form id="disposisiForm" action="{{ route('admin.pengajuan-keberatan.disposisi.store', $pengajuan->id_pengajuan) }}" method="POST">
            @csrf
            ...
            <div class="flex gap-4 justify-end">
                <a href="{{ route('admin.pengajuan-keberatan.show', $pengajuan->id_pengajuan) }}"
        ...
        <x-confirmation-dialog
            ...
            description="Apakah Anda yakin ingin mendisposisikan pengajuan keberatan ini ke SKPD yang dipilih?"
            theme="primary"
        ...
        <x-notification-modal
            ...
            :description="session('success') ?? 'Pengajuan keberatan berhasil didisposisikan.'"
        />
    </div>
</x-admin-layout>
