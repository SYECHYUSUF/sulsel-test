        ...
        <div class="glass-card dark:glass-card-dark rounded-2xl overflow-hidden shadow-xl">
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-700/50 dark:to-slate-800/50 border-b-2 border-slate-200/50 dark:border-slate-600/50 text-slate-700 dark:text-slate-300 font-bold">
                        <tr class="bg-gradient-to-r from-slate-50/50 to-slate-100/50 dark:from-slate-800/50 dark:to-slate-700/50">
                    ...
                    <tbody class="divide-y divide-slate-100/50 dark:divide-slate-700/50">
                        ...
                        <template x-if="loading">
                            <template x-for="i in 5" :key="i">
                                <tr class="animate-pulse">
                                    ...
                                    <td class="px-6 py-4"><div class="h-4 bg-slate-200 dark:bg-slate-700 rounded w-2/3"></div></td>
                                    <template x-if="isAdmin"><td class="px-6 py-4"><div class="h-4 bg-slate-200 dark:bg-slate-700 rounded w-1/2"></div></template>
                                    <td class="px-6 py-4"><div class="h-4 bg-slate-200 dark:bg-slate-700 rounded w-1/2"></div></td>
                                    <td class="px-6 py-4"><div class="h-6 bg-slate-200 dark:bg-slate-700 rounded-full w-20"></div></td>
                                    <td class="px-6 py-4"><div class="h-4 bg-slate-200 dark:bg-slate-700 rounded w-24"></div></td>
                                    <td class="px-6 py-4 text-right"><div class="h-8 bg-slate-200 dark:bg-slate-700 rounded-lg w-10 ml-auto"></div></td>
                                </tr>
                            </template>
                        </template>
                        ...
                        <template x-if="!loading">
                            <template x-for="item in items" :key="item.id_pengajuan">
                                <tr class="hover:bg-white/50 dark:hover:bg-slate-700/30 transition-all duration-200 animate-fade-in">
                                    <td class="px-6 py-4">
                                        <div>
                                            <div class="font-semibold text-slate-900 dark:text-slate-100" x-text="item.nama_pemohon"></div>
                                    ...
                                    <template x-if="isAdmin">
                                        <td class="px-6 py-4">
                                            <template x-if="item.skpd">
                                                <span class="inline-flex items-center gap-2 px-3 py-1 rounded-lg bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs font-medium">
                                            ...
                                            <template x-if="!item.skpd">
                                                <a :href="'/admin/pengajuan-keberatan/' + item.id_pengajuan + '/disposisi'" 
                                                   class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 text-xs font-medium hover:bg-amber-100 dark:hover:bg-amber-900/50 transition-colors border border-amber-200 dark:border-amber-800">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                                                    </svg>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-slate-700 dark:text-slate-300 line-clamp-2 max-w-xs" x-text="item.kasus || 'Tidak ada alasan'"></div>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-slate-600 dark:text-slate-400 text-sm" x-text="formatDate(item.created_at)"></div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex justify-end gap-2">
                                            ...
                                            <button @click="confirmDelete(item.id_pengajuan)"
                                               ...
                                               x-show="['a', 'y', 't'].includes(item.status)">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <!-- Pagination -->
            <div class="px-6 py-4 border-t border-slate-200/50 dark:border-slate-700/50 bg-white/30 dark:bg-slate-800/30" x-show="!loading && items.length > 0">
                <div class="flex flex-col md:flex-row items-center justify-between gap-4">
                    <div class="text-sm text-slate-600 dark:text-slate-400">
                        Menampilkan <span class="font-bold text-slate-900 dark:text-slate-100" x-text="items.length"></span> dari <span class="font-bold text-slate-900 dark:text-slate-100" x-text="pagination.total"></span> data
                    </div>
                    <div class="flex items-center gap-2">
        ...
        <div x-show="showFeedbackModal" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="showFeedbackModal" 
                    ...
                    <form :action="'/admin/pengajuan-keberatan/' + (selectedItem ? selectedItem.id_pengajuan : '') + '/feedback'" method="POST">
                        @csrf
                        <div class="px-6 pt-6 pb-4">
                            ...
                            <div class="flex items-center justify-between mb-6">
                                ...
                                <button type="button" @click="showFeedbackModal = false" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
                            ...
                            <!-- Detail Section -->
                            <div class="bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-900 p-5 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 mb-6 max-h-96 overflow-y-auto">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div class="space-y-1">
                                        <dt class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Nomor Pendaftaran</dt>
                                        <dd class="text-base font-bold text-slate-900 dark:text-slate-100" x-text="detailItem.no_pendaftaran || '-'"></dd>
                                    </div>
                                    <div class="space-y-1">
                                        <dt class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Nama Pemohon</dt>
                                        <dd class="text-base font-bold text-slate-900 dark:text-slate-100" x-text="detailItem.nama_pemohon || '-'"></dd>
                                    </div>
                                    <div class="space-y-1">
                                        <dt class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Nomor Telepon</dt>
                                        <dd class="text-base font-medium text-slate-700 dark:text-slate-300" x-text="formatPhone(detailItem.no_telp_pemohon)"></dd>
                                    </div>
                                    <div class="space-y-1">
                                        <dt class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Metode Respon</dt>
                                        <dd class="text-base font-medium text-slate-700 dark:text-slate-300">
                                            <span x-show="detailItem.metode_respon === 'whatsapp'" class="inline-flex items-center gap-1 px-2.5 py-1 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded-lg text-xs font-semibold">
                                            ...
                                            <span x-show="detailItem.metode_respon === 'website' || !detailItem.metode_respon" class="inline-flex items-center gap-1 px-2.5 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-lg text-xs font-semibold">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <div class="md:col-span-2 space-y-1">
                                        <dt class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Alasan Keberatan</dt>
                                    ...
                                    <div class="md:col-span-2 space-y-1">
                                        <dt class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Kasus Posisi</dt>
                                        <dd class="text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap bg-white/50 dark:bg-slate-800/50 p-3 rounded-lg" x-text="detailItem.kasus || 'Tidak ada keterangan'"></dd>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-6 p-4 bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 border border-green-200 dark:border-green-800 rounded-xl">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div class="text-sm font-bold text-slate-900 dark:text-slate-100" x-text="detailItem.nama_pemohon"></div>
                                            <div class="text-xs text-slate-600 dark:text-slate-400" x-text="formatPhone(detailItem.no_telp_pemohon)"></div>
                                        </div>
                                    </div>
                                    <a :href="getWhatsAppLink(detailItem)" 
                                       target="_blank" 
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Balasan Admin</label>
                                ...
                                <div x-show="existingFeedback" class="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl">
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div class="flex-1">
                                            <p class="text-xs font-semibold text-blue-900 dark:text-blue-200 mb-1">Balasan Sebelumnya:</p>
                                            <p class="text-sm text-blue-800 dark:text-blue-300" x-text="existingFeedback"></p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <!-- Modal Footer -->
                        <div class="px-6 py-4 bg-slate-50/50 dark:bg-slate-800/50 border-t border-slate-200/50 dark:border-slate-700/50 flex justify-end gap-3">
                            <button type="button" 
                                    @click="showFeedbackModal = false" 
