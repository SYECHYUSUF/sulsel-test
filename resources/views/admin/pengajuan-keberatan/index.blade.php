<x-admin-layout>
    ...
    <div x-data="keberatanDataTable()" x-init="fetchData()" class="space-y-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Pengajuan Keberatan</h1>
                <p class="text-slate-500 text-sm">Kelola keberatan atas permohonan informasi publik.</p>
            </div>
                        ...
                        <template x-if="loading">
                            <template x-for="i in 5" :key="i">
                                <tr class="animate-pulse">
                                    ...
                                    <template x-if="isAdmin"><td class="px-6 py-4"><div class="h-4 bg-slate-100 rounded w-1/2"></div></td></template>
                        ...
                        <template x-if="!loading">
                            <template x-for="item in items" :key="item.id">
                                <tr class="hover:bg-slate-50/50 transition-colors">
                                    <td class="px-6 py-4">
                                        <div class="font-medium text-slate-900" x-text="item.nama_pemohon"></div>
                                        <div class="text-xs text-slate-400" x-text="'Kode: ' + (item.kode_permohonan || '-')"></div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-slate-600 line-clamp-1" x-text="item.alasan_keberatan"></div>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <a :href="'/admin/pengajuan-keberatan/' + item.id" class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all inline-block">
