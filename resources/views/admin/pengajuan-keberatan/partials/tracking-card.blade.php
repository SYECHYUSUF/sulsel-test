            ...
            @if($pengajuan->disposisi->isNotEmpty())
                <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
                    <div class="p-6 border-b border-slate-100 dark:border-slate-700">
                        <h3 class="text-lg font-bold text-ppid-primary dark:text-blue-400">Tracking Disposisi OPD</h3>
                        <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">Status dan respon dari setiap OPD yang menerima disposisi</p>
                    </div>
                    <div class="p-6 space-y-4">
                        @foreach($pengajuan->disposisi as $disp)
                            <div class="border border-slate-200 dark:border-slate-700 rounded-lg p-4">
                                ...
                                @if($disp->respon->isNotEmpty())
                                    @foreach($disp->respon as $resp)
                                         <div class="bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800 rounded-lg p-3">
                                            <p class="text-xs font-semibold text-green-800 dark:text-green-300 mb-1">
                                                Respon OPD @if($resp->created_at)({{ $resp->created_at->format('d M Y H:i') }})@endif:
                                            </p>
                                            <p class="text-sm text-slate-700 dark:text-slate-300">{{ $resp->isi_respon }}</p>
                                    ...
                                    @endforeach
                                @else
                                    {{-- Response Form for OPD --}}
                                    @if(Auth::user()->hasRole('opd') && Auth::user()->id_skpd === $disp->id_skpd && $disp->status !== 'selesai' && $disp->status !== 'ditolak')
                                        <form action="{{ route('admin.pengajuan-keberatan.respon.store', $disp->id_disposisi) }}" 
                                              method="POST" 
                                            ...
                                            <div class="space-y-3">
                                                ...
                                                <div>
                                                    ...
                                                    <textarea name="respon" rows="4" required
                                                        class="w-full px-3 py-2 text-sm border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 rounded-lg focus:ring-2 focus:ring-ppid-primary"
                                                        placeholder="Tuliskan respon atau jawaban untuk pengajuan keberatan ini..."></textarea>
                                                </div>
                                    ...
                                    @else
                                        <p class="text-slate-500 dark:text-slate-400 italic text-sm">Belum ada respon dari OPD ini.</p>
                                    @endif
                                @endif
                            </div>
                        @endforeach
