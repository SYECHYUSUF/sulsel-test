...
<script>
    ...
    document.getElementById('form-pekerjaan-create').addEventListener('submit', function (e) {
        e.preventDefault();
        const formData = new FormData(this);
        const data = {
            nama_pekerjaan: formData.get('nama_pekerjaan'),
            is_active: formData.get('is_active') ? 1 : 0
        };

        fetch('{{ route('admin.master-data.pekerjaan.store') }}', {
            method: 'POST',
            headers: {
    ...
    document.getElementById('form-pekerjaan-edit').addEventListener('submit', function (e) {
        e.preventDefault();
        const formData = new FormData(this);
        const id = document.getElementById('pekerjaan-id').value;
        const data = {
            nama_pekerjaan: formData.get('nama_pekerjaan'),
            is_active: formData.get('is_active') ? 1 : 0
        };

        fetch(`/admin/master-data/pekerjaan/${id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
