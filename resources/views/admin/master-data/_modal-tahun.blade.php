...
<script>
    ...
    document.getElementById('form-tahun-create').addEventListener('submit', function (e) {
        e.preventDefault();
        ...
        fetch('{{ route('admin.master-data.tahun.store') }}', {
            method: 'POST',
            headers: {
    ...
    document.getElementById('form-tahun-edit').addEventListener('submit', function (e) {
        e.preventDefault();
        const formData = new FormData(this);
        ...
        fetch(`/admin/master-data/tahun/${id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
