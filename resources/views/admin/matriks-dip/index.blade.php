<x-admin-layout title="Daftar Informasi Publik - Admin PPID">
    <div class="flex items-center justify-between mb-4">
        ...
        <a href="{{ route('admin.matriks-dip.create') }}"
            class="px-4 py-2 bg-ppid-primary dark:bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-ppid-dark dark:hover:bg-blue-700 transition-colors flex items-center gap-2">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden"
        ...
        <div
            class="p-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 flex justify-between">
            <div class="relative w-full md:w-64">
                <form action="{{ route('admin.matriks-dip.index') }}" method="GET">
                    <input type="text" name="search" value="{{ request('search') }}"
                        class="w-full pl-10 pr-4 py-2 border border-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 rounded-lg text-sm focus:outline-none focus:border-ppid-accent focus:ring-1 focus:ring-ppid-accent placeholder-slate-400 dark:placeholder-slate-500"
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.matriks-dip.edit', $item->id) }}"
                                        class="p-2 text-slate-500 dark:text-slate-400 hover:text-ppid-primary dark:hover:text-blue-400 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-lg transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <button type="button"
                                        @click="confirmDelete('{{ route('admin.matriks-dip.destroy', $item->id) }}')"
        ...
        <x-confirmation-dialog trigger="showDeleteModal" title="Hapus Data?"
            description="Data yang dihapus tidak dapat dikembalikan secara permanen." confirmText="Ya, Hapus"
            theme="danger" url="deleteUrl" :dynamic="true" method="DELETE" />
    </div>
</x-admin-layout>