<x-admin-layout title="Edit Kategori Informasi - Admin PPID">
    <x-slot name="header">
        <div class="flex items-center gap-2">
            ...
            <span>Edit Kategori Informasi</span>
        </div>
    </x-slot>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden max-w-2xl mx-auto">
        ...
        <form action="{{ route('admin.kategori-informasi.update', $kategori->id_kat_info) }}" method="POST"
            ...
            <div>
                ...
                <input type="text" name="nm_kat_info" value="{{ old('nm_kat_info', $kategori->nm_kat_info) }}"
            ...
            <div>
                ...
                <select name="is_active"
                    ...
                    <option value="0" {{ old('is_active', $kategori->is_active) == '0' ? 'selected' : '' }}>Tidak Aktif
                    </option>
                </select>
                @error('is_active')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
